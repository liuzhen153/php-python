...
#    新增日志记录，脚本控制启动/重启/停止，支持supervisor管理ppython进程 -- by Tommy 2019-03-29
#    新增ipv6支持 -- by Tommy 2019-04-03
# -------------------------------------------------------------------------------
    ...
    def run(self):
        ...
        if (modul not in pc_dict):  # 预编译字典中没有此编译模块
            ...
            try:
                ...
                callMod = __import__(modul, fromlist=(
                    modulname,))  # 根据module名，反射出module
                pc_dict[modul] = callMod  # 预编译字典缓存此模块
            except Exception as e:
                logger.loginfo('模块不存在:%s' % modul)
                self._socket.sendall(
                    ("F" + "module '%s' is not exist or there is an error in your .py file!" % modul).encode(php_python.CHARSET))  # 异常
                self._socket.close()
                return
        else:
        ...
        try:
            ...
            self._socket.sendall(
                ("F" + "function '%s()' is not exist or there is an error in your .py file!" % func).encode(php_python.CHARSET))  # 异常
            self._socket.close()
            return
